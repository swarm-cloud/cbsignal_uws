...
class Server {
        ...
        this.app
            ...
            .get("/", (response, request) => {
                ...
                if (client) {
                    ...
                    client = new Client(response, id, true, this.compact.enable && compactVersion === COMPACT_VERSION);
            ...
            .ws("/*", {
                ...
                maxBackpressure: 128 * 1024,  // TODO 验证
                idleTimeout: 300,
    ...
    _onOpen(ws) {
        const { id, token, compactVersion } = ws.getUserData();
        if (!id || id.length < 6) {
        ...
        if (this.hub.hasClient(id)) {
            ...
            client = new Client(ws, id, false, this.compact.enable && compactVersion === COMPACT_VERSION)
            this.hub.doRegister(client);
        }
    ...
    _onMessage(ws, message) {
        ...
        const { id, compactVersion } = ws.getUserData();
