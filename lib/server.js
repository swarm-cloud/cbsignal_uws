...
class Server {
        ...
        this.app
            ...
            .get("/", (response, request) => {
                ...
                if (client) {
                    ...
                    client = new Client(response, id, true, this.compact.enable && compactVersion === COMPACT_VERSION, device);
    ...
    _onUpgrade(res, req, context) {
        ...
        res.upgrade(
            { id, token, compactVersion, device },
            secWebSocketKey,
    ...
    _onOpen(ws) {
        const { id, token, compactVersion, device } = ws.getUserData();
        if (!id || id.length < 6) {
        ...
        if (client) {
            ...
            client = new Client(ws, id, false, this.compact.enable && compactVersion === COMPACT_VERSION, device)
            this.hub.doRegister(client);
        }
    ...
    _onMessage(ws, message) {
        ...
        const { id, compactVersion, device} = ws.getUserData();
