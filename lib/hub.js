...
class Hub {
    ...
    sendMessageToLocalPeer(raw) {
        ...
        for (let item of object.items) {
            if (client && client.peerId === item.toPeerId) {
                // this.logger.warn(`hit peer ${item.toPeerId} cache`);   // TODO 验证
            } else {
    ...
    processMessage(json, peerId) {
        ...
        if (action === "ping" || action === "heartbeat") {
            ...
            if (action === "signal") {
                this.processSignal(target, json, toPeerId, peerId, key);
            } else if (action === "signals") {
                this.processSignals(target, json, toPeerId, peerId, key);
            } else if (action === "reject") {
                this.processReject(target, json, toPeerId, key);
            } else {
    ...
    async processSignals(target, json, toPeerId, peerId, key) {
        ...
        for (let item of data) {
            ...
            if (!await this.processSignal(target, msg, toPeerId, peerId, key)) {
                break
            }
        }
    }

    async processSignal(target, json, toPeerId, peerId, key) {
        if (target) {
    ...
    async processReject(target, json, toPeerId, key) {
        if (target) {
