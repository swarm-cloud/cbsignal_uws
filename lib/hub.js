...
class Hub {
    ...
    processMessage(json, peerId) {
        ...
        if (action === "ping" || action === "heartbeat") {
            ...
            if (action === "signal") {
                this.processSignal(target, json, toPeerId, peerId);
            } else if (action === "signals") {
                this.processSignals(target, json, toPeerId, peerId);
            } else if (action === "reject") {
                this.processReject(target, json, toPeerId);
            } else {
    ...
    async processSignal(target, json, toPeerId, peerId) {
        if (target) {
    ...
    async processReject(target, json, toPeerId) {
        if (target) {
