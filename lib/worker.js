...
const { getCertInfo, getLocalIp} = require("./utils/tool");
...
const startWorker = async (config = {}) => {
    ...
    const nodes = new Nodes(getLocalIp(), logger);
    ...
    if (config.redis) {
        ...
        if (isCluster) {
            ...
            await redis.connectCluster(cluster, username, password);
        } else {
            logger.info(`${process.pid} start connect redis ${host}:${port} db ${dbname}`);
            await redis.connect(host, port, username, password, dbname);
        }
