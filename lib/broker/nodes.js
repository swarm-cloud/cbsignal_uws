...
class Nodes {
    constructor(selfAddr, logger, msgConsumeInterval, maxPipeLen) {
        this._map = new Map();
        this.selfAddr = selfAddr;
        this.logger = logger;
        this.msgConsumeInterval = msgConsumeInterval;
        this.maxPipeLen = maxPipeLen;
    }
    ...
    async get(addr) {
        ...
        if (!node) {
            ...
            if (count >= 0) {
                node = new Node(addr, this.logger, this.msgConsumeInterval, this.maxPipeLen);
                this.add(addr, node);
            } else {
                return null
            }
